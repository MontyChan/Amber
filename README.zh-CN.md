...
每次归档任务最多生成两个 `.7z` 文件：
- **压缩包：** 用于存储常规可压缩文本与结构化数据
- **存储包（仅存储）：** 针对已压缩或高熵文件，避免重复无用压缩，提升打包效率

**元数据与实体解耦：**
归档元数据独立持久化于 SQLite 数据库中，而非固化在压缩包内部。当归档资产规模增长后，无需逐个挂载或读取物理压缩包，即可直接实现跨归档的路径、备注及历史记录的高效全局检索
