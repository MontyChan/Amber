...
Amber 是一个用于本地冷存档场景的 CLI/TUI 工具。项目使用 7z 生成归档包，使用 SQLite 保存归档元数据与文件索引，使已封存目录仍可继续进行搜索、浏览、结构检查与按需恢复。
...
与仅保留压缩文件的归档方式不同，Amber 将归档实体与索引信息解耦存储，在归档数量增长后仍可直接进行跨归档检索，而无需逐个读取压缩包内容。
